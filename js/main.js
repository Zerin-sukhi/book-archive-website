...
const getBookData = (booksobj) => {
  ...
  if (getBookDataArray.length === 0) {
    document.getElementById('show-validity').innerText = 'Did not find any result,Try another one';
  }
  ...
  getBookDataArray.forEach(book => {
    //clear valid mesege
    document.getElementById('show-validity').innerText = '';
    //book name
    const bookName = book.title;
    //book author name
    const bookAuthorName = book.author_name;
    //book publish date
    const bookFirstPub = book.publish_date;
    //image id
